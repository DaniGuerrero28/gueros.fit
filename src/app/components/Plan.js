export default function Plan({ title, price, originalPrice, description, features, image, ctaLabel, ctaHref, highlight = false, badge = null, custom = false, paymentText = '' }) {
  return (
    <div
      className={`relative rounded-3xl p-8 flex flex-col items-stretch max-w-sm w-full transition-all duration-200 gap-4
        ${highlight ? 'bg-foreground text-primary-foreground shadow-2xl scale-105 z-10' : ' bg-primary-foreground text-primary  shadow'}
    ...
    >
      ...
      {/* Título */}
      <h2 className={`text-2xl font-bold font-montserrat ${highlight ? 'text-primary-foreground' : 'text-primary' }`}>{title}</h2>
      {/* Descripción */}
      <p className={`text-base ${highlight ?  'text-secondary': 'text-secondary-foreground' }`}>{description}</p>
      {/* Features */}
      <ul className={`text-sm space-y-2 w-full h-60 md:h-64 lg:h-52 xl:h-44 max-w-xs mx-auto ${highlight ? 'text-primary-foreground' :  'text-primary'}`}>
